function [blanktime] = blank_kal(data_all,user,time,interval)
%ACCURACY_KAL 此处显示有关此函数的摘要
%   此处显示详细说明

probability=probability_kal(data_all,user,time,interval);
probability=fovtile(probability);

data_pt=Pretreatment(data_all,time+interval);
viewpoint=data_pt(:,:,user);
p=ones([length(viewpoint),1]);
blanktime=0;
for i=1:length(viewpoint)
    ...
    for x=xfov
        for y=yfov
            if p(i)>=probability(x,y)
                p(i)=probability(x,y);
            end
        end
    end
    if p(i)<0.1/15
        blanktime=blanktime+1/length(viewpoint);
    end
end
end
